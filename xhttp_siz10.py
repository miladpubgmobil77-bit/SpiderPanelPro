...
def _xhttp_match_custom_path(path: str):
    """Match only the stable configured base path + user UUID.

    Xray sends the session and sequence metadata in X-Session/X-Seq headers in
    the current panel-generated links, so transport mode is inferred from HTTP
    method and the presence of X-Seq rather than being encoded into the path.
    The legacy /base/{mode}/{uuid}/{session} form remains accepted below.
    """
    import main as m
    p=(path or '').strip('/')
    ...
    for ib in m.INBOUNDS.values():
        ...
        if len(rest)==1 and rest[0]:
            # New canonical form: /base/{uuid} + X-Session/X-Seq headers.
            return 'auto', rest[0], None, None
        # Legacy compatibility: /base/{mode}/{uuid}/{session}[/{seq}]
...
def _xhttp_headers_meta(request: Request):
    session = str(request.headers.get('x-session') or request.headers.get('x-session-id') or '').strip()
    seq_raw = str(request.headers.get('x-seq') or '').strip()
    try:
...
async def xhttp_custom_path_get(path: str, request: Request):
    ...
    if mode == 'auto':
        session, seq = _xhttp_headers_meta(request)
        if not session: raise HTTPException(status_code=400,detail='X-Session header required')
        return await xhttp_downlink('packet-up',uuid,session,request)
