...
async def _download_official_files() -> tuple[Path, Path]:
    ...
    if not config_file.exists() or (time.time() - config_file.stat().st_mtime) > 86400:
...
class MTProtoProxyServer:
    ...
    MTProxy listens on the exact Internal Port selected for its Telegram inbound.
    Railway public/external TCP settings are separate and never override this value.
    """
    ...
    def __init__(self, inbound_id: str, port: int, sni: str = "", destination: str = "", server_name: str = "", promotion_tag: str = ""):
        ...
        if self.port < 1 or self.port > 65535:
            raise ValueError(f"Invalid MTProto listener port: {self.port}")
        # Railway application/public routing is separate metadata. It must not
        # overwrite or redefine this MTProxy listener port. The listener always
        # binds exactly to the Telegram inbound Internal Port.
        if railway_app_port and int(railway_app_port) == self.port:
            logger.info("[TG Proxy %s] Railway TCP application port matches internal listener: %s", inbound_id, self.port)
        elif railway_app_port:
            logger.info("[TG Proxy %s] Railway application port=%s; MTProxy internal listener remains=%s (external endpoint is separate)", inbound_id, railway_app_port, self.port)
    ...
    async def start(self):
        ...
        # Verify the exact internal listener requested by the inbound.
        # Railway's public TCP proxy can expose 443; that must never be confused
        # with the application-side MTProto listen port.
        try:
            import subprocess
            probe = await asyncio.to_thread(
                subprocess.run,
                ["ss", "-ltnH", "sport", "=", f":{self.port}"],
                capture_output=True, text=True, timeout=3,
            )
            if probe.returncode == 0 and not probe.stdout.strip():
                logger.warning(
                    "[TG Proxy %s] process started but port %s is not visible in ss; check binary/runtime",
                    self.inbound_id, self.port,
                )
            else:
                logger.info("[TG Proxy %s] verified TCP listener on internal port %s", self.inbound_id, self.port)
        except Exception as exc:
            logger.debug("[TG Proxy %s] listener verification skipped: %s", self.inbound_id, exc)
        logger.info("[TG Proxy %s] MTProxy is listening on internal port %s (external/public port is separate)", self.inbound_id, self.port)
